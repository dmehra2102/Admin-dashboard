...
export const CreateCompany = () => {
  ...
  return (
    <CompanyList>
      <Modal {...modalProps} mask={true} onCancel={gotToListPage} title="Create Company" width={512}>
        <Form {...formProps} layout="vertical">
          <Form.Item label="Company name" name={"name"} rules={[{ required: true }]}>
            <Input placeholder="Plese enter a company name" />
          </Form.Item>
          <Form.Item label="Sales owner" name={"salesOwnerId"} rules={[{ required: true }]}>
            <Select
              placeholder="Plese select a sales owner"
              {...selectProps}
              options={
